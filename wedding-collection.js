...
function load() {
    $.ajax({
        ...
        success: function (res) {
            ...
            // Remove the limitation on productsToShow
            for (let i = res.length - 1; i >= 0; i--) {
                let d = res[i];
                if (d.category == "Neophyte-men-wedding-collection") {
                    let html = render(d);
                    $('#men-wedding-products').append(html);
                }
            }
...
$(document).ready(function () {
    ...
    $('#men-wedding-products').on('click', '.wish-list', function () {
